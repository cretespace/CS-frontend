      ...
      <form  className='p-10 rounded-[18.94px] flex flex-col gap-[28px]   [background:linear-gradient(to_right,_#00000030_0%,_#062d0831_10%,_#062D0866_70%,_#00000030_100%)] border-[0.95px] border-[#0C520A80]'>
        ...
        <div className="flex items-center justify-cemter gap-[16px] rounded-[5px] bg-black/15 w-[351px] h-[32px] px-[8px] py-[4px]">
          {accountTypeArray.map((item, index) => (
            <p key={index} onClick={()=>setAccountType(item)} className={accountType === item ? "rounded-[3px] py-[5px] h-[24px] text-center flex items-center  px-[9px] bg-[#013500] flex items-center cursor-pointer" : "text-white/50 cursor-pointer"}>
              {item}
            </p>
          ))}
        </div>}
        ...
        { (accountType !== 'Student' && accountType !== 'Business Center') 
          ...
          {signup && <div className='flex gap-[8px] -mt-3'>
            ...
            <p className='text-white/50 font-inter text-[12px]'>By creating an account, I agree to the <span className='underline text-white/70 my-3'>Terms & Conditions </span>and <span className='underline text-white/70 my-3'>Privacy Policy</span></p>
          </div> }
